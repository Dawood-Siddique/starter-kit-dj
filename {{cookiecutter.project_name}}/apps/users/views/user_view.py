...
class SendOtpView(APIView):
    ...
    def post(self, request):
        ...
        if user.generate_otp():
            ...
            return Response({'message': 'Error, OTP not sent'}, status=status.HTTP_503_SERVICE_UNAVAILABLE)
...
class EnableMfaView(APIView):
    ...
    def post(self, request):
        ...
        return Response({'message': 'MFA enabled successfully', 'uri': uri}, status=status.HTTP_200_OK)
...
class VerifyMfaView(APIView):
    ...
    def post(self, request):
        ...
        if not mfa_token or not otp:
            return Response({'message': 'mfa_token and otp are required'}, status=status.HTTP_400_BAD_REQUEST)

        # Find user by mfa_token
        try:
            user = User.objects.get(mfa_token=mfa_token)
        except User.DoesNotExist:
            return Response({'message': 'Invalid or expired MFA token'}, status=status.HTTP_401_UNAUTHORIZED)

        # Verify the mfa_token is still valid (not expired)
        if not user.verify_mfa_token(mfa_token):
            return Response({'message': 'MFA token expired. Please login again.'}, status=status.HTTP_401_UNAUTHORIZED)

        if not user.mfa_secret or not user.mfs_enabled:
            return Response({'message': 'MFA is not enabled for this user'}, status=status.HTTP_400_BAD_REQUEST)
...
class DisableMfaView(APIView):
    ...
    def post(self, request):
        ...
        if not user.mfs_enabled:
            return Response({'message': 'MFA is not enabled'}, status=status.HTTP_400_BAD_REQUEST)

        # Verify OTP before disabling for security
        if not otp:
            return Response({'message': 'OTP is required to disable MFA'}, status=status.HTTP_400_BAD_REQUEST)
...
class GoogleSignInView(APIView):
    ...
    Returns the same JWT payload as the regular login endpoint.
    """
